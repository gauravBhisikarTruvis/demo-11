...
def node_context_enrichment(state: OptimizerState) -> OptimizerState:
    ...
    # For production, use sqlglot or similar. Here we use a simple heuristic for demonstration.
    import re
    # Matches `project.dataset.table` or `dataset.table`
    potential_tables = re.findall(r'(?:FROM|JOIN)\s+`?([a-zA-Z0-9_\-\.]+)(?:\s|`|$)', query, re.IGNORECASE)
    
    if not potential_tables:
        logger.info("No tables found in query to look up.")
        state['tables_involved'] = []
        return state
        
    state['tables_involved'] = potential_tables
